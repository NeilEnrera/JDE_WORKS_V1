...
// Store Step 3 data (payment_option) in session
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    if (isset($_POST['payment_option'])) {
        $_SESSION['payment_option'] = $_POST['payment_option'];
        
        // Skip Step 4 if Full Payment is selected
        if ($_POST['payment_option'] === 'full') {
            $_SESSION['payment_method'] = 'gcash'; // Default to GCash for Full Payment
...
// Block access to Step 4 if Full Payment is selected (for GET requests)
if (isset($_SESSION['payment_option']) && $_SESSION['payment_option'] === 'full') {
    header('Location: upload_proof.php');
    exit();
}
...
require '../html/payment_method.view.php';